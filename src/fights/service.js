...
class FightService {
  ...
  async runFightFromReactionMessage(interaction, messageLink) {
    ...
    if (this.hasActiveFight(interaction.guildId)) {
      await interaction.editReply({ content: 'There is already an active ME/CFS Warriors fight in this server.' });
      return;
    }

    const parsedLink = this.parseDiscordMessageLink(messageLink);
    if (!parsedLink) {
      await interaction.editReply({ content: 'Invalid Discord message link. Use the full `https://discord.com/channels/...` link.' });
      return;
    }

    if (parsedLink.guildId !== interaction.guildId) {
      await interaction.editReply({ content: 'The message link must point to a message in this server.' });
      return;
    }

    const channel = await this.fetchChannel(parsedLink.channelId);
    if (!channel?.isTextBased?.()) {
      await interaction.editReply({ content: 'Could not access the message channel from that link.' });
      return;
    }

    const message = await channel.messages.fetch(parsedLink.messageId).catch(() => null);
    if (!message) {
      await interaction.editReply({ content: 'Could not fetch that message. Check the link and the bot permissions.' });
      return;
    }

    const players = await this.collectReactionPlayers(message);
    if (players.length < MIN_PLAYERS) {
      await interaction.editReply({
        content: `Not enough reacted players found on that message. Need at least ${MIN_PLAYERS} human players.`
      });
      return;
    }
    ...
    const state = createFightState({
      ...
      hostMention: getDisplayTextFromInteraction(interaction),
      type: 'staff',
      lobbyMessageId: parsedLink.messageId
    });

    state.participants = new Map(players.map(player => [player.id, player]));
    this.activeFights.set(interaction.guildId, state);

    await interaction.editReply({
      content: `Recovered ${players.length} reacted players from ${message.url}. Starting the ME/CFS Warriors fight now in <#${parsedLink.channelId}>.`
    });

    try {
      await this.startFight(state);
    } catch (err) {
      this.activeFights.delete(interaction.guildId);
      throw err;
    }
  }
  ...
  parseDiscordMessageLink(link) {
    const match = /^https?:\/\/(?:canary\.|ptb\.)?discord(?:app)?\.com\/channels\/(\d+)\/(\d+)\/(\d+)\/?$/.exec(link.trim());
    if (!match) {
      return null;
    }
  ...
  async collectReactionPlayers(message) {
    const joinReaction = message.reactions.cache.find(entry => entry.emoji.name === JOIN_EMOJI);
    const players = [];

    if (joinReaction) {
      const users = await joinReaction.users.fetch().catch(() => null);
      if (users) {
        for (const [, user] of users) {
          ...
          const player = await this.buildPlayer(message.guild, user.id);
          if (player) {
            players.push(player);
          }
        }
